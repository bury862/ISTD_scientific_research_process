...
###################################################
class tensor_dilate(nn.Module):  # 大眼效果
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        if stride != 1 or out_channels != in_channels:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride), #改变通道数
...
class HDNet(nn.Module):
    def __init__(self, input_channels, block=ResNet):
        ...
        self.dilate = tensor_dilate()###新增大眼效果
        ...
        self.conv_init = nn.Conv2d(7, param_channels[0], 1, 1)#改变通道数
        self.py_init = self._make_layer2(7, 1, block)#改变通道数
    ...
    def forward(self, x, warm_flag):
        ...
        x = torch.cat([x, x_gard, x_dilate], 1)
