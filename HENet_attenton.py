...
class FrequencyChannelAttention(nn.Module):
    def __init__(self, in_channels, reduction=16):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Conv2d(in_channels, max(1, in_channels // 16), 1, bias=False),
            nn.ReLU(inplace=True),
            #nn.Conv2d(in_channels // reduction, in_channels, 1, bias=False),
            nn.Conv2d(max(1, in_channels // 16), in_channels, 1, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        # Global Avg Pool on amplitude spectrum
        y = self.avg_pool(amplitude)
        y = self.fc(y)
        return x * y
...
class ResNet(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # 添加通道注意力
        self.fca = FrequencyChannelAttention(out_channels)
