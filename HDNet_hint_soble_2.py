...
from model.hint import Hint, Sobel#要有文件夹名（（
...
class HDNet(nn.Module):
    ...
    def forward(self, x, warm_flag):
        ...
        # 初始化 Sobel 卷积模块
        sobel = self.sobel(x)
        x_0 = torch.cat([x_p, sobel], dim=1)  # 拼接起来
        x_0 = self.hintx_proj(x_0)  # 融合拼接数据    通道16

        hint = self.hint(x_0)  # 预处理一下
        x_1 = torch.cat([x_p, hint], dim=1)  # 拼接起来
        x_1 = self.hintx_proj(x_1)  # 融合拼接数据    通道16

        #x = torch.cat([x_1, x_2], dim=1)  # 拼接起来
