...
class SobelConv(nn.Module):#边缘检测
    def __init__(self):
...
class HDNet(nn.Module):
    def __init__(self, input_channels, block=ResNet):
        ...
        self.conv_before_init = nn.Conv2d(input_channels, param_channels[0], 1, 1)
        self.final_conv = nn.Conv2d(2*input_channels+param_channels[0], param_channels[0], kernel_size=3, padding=1)
        ...
        self.py_init = self._make_layer2(16, 1, block)  #修改通道数
    ...
    def forward(self, x, warm_flag):
        ...
        # 获取 Sobel 卷积的边缘特征
        edges = self.sobel_conv(x)
        x_init = self.conv_before_init(x)
        #拼接
        x_fuse = torch.cat([x_init, edges], dim=1)
        #卷积融合一下
        x = self.final_conv(x_fuse)
